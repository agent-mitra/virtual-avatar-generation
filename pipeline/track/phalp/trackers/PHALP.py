...
class PHALP(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        sam_checkpoint = os.path.abspath("phalp/trackers/sam_vit_h_4b8939.pth") #"sam_vit_h_4b8939.pth"
        model_type = "vit_h"
        ...
        yolo_checkpoint = os.path.abspath("phalp/trackers/yolov8l-seg.pt")
    ...
    def track(self):
        ...
        list_of_frames, additional_data, list_of_orig_frames = io_data['list_of_frames'], io_data['additional_data'], io_data['list_of_orig_frames']
        ...
        try: 
            ...
            print(f"final frame is {list_of_frames[-1]}")
            for t_, frame_name in progress_bar(enumerate(list_of_frames), description="Tracking : " + self.cfg.video_seq, total=len(list_of_frames), disable=False):
                if (t_ > 50 and self.cfg.video.inference == True):
                    return
                image_frame               = self.io_manager.read_frame(frame_name)
                orig_image_frame = self.io_manager.read_frame(list_of_orig_frames[t_])
                video_name = self.cfg.video.source.split('/')[-1].split('.')[0]
                ...
                pred_bboxes_into_sam, pred_masks_orig, _, _, _, _ = self.get_detections(orig_image_frame, list_of_orig_frames[t_], t_, additional_data, measurments)
                self.person_predictor.set_image(orig_image_frame)
                allMasks = []
                for box in pred_bboxes_into_sam:
                    input_box = np.array(box)
                    masks, _, _ = self.person_predictor.predict(
                        point_coords=None,
                        point_labels=None,
                        box=input_box[None, :],
                    ...
                    if len(masks) > 0:
                        ...
                        mask = np.zeros_like(orig_image_frame)
                    allMasks.append(mask)
                
                for segMask in pred_masks_orig: 
                    allMasks.append(segMask)
                    
                results = self.model(list_of_orig_frames[t_])
                classes_xy_masks = []
                if results[0].masks:
                ...
                for boundary in classes_xy_masks:
                    imageA_mask = np.zeros_like(orig_image_frame[..., 0])
                ...
                #del classes_xy_masks
                # torch.cuda.empty_cache()
                # gc.collect()
                if len(allMasks) > 0:
                    combined_mask = np.zeros_like(allMasks[0], dtype=bool)
                else:
                    print("No people detected here. Removing frame..")
                    newFrameName = frame_name.split('/')[-1].split('.')[0]
                    origDir = self.cfg.video.output_dir + "/_DEMO/" + video_name + "/orig"
                    imgDir = self.cfg.video.output_dir + "/_DEMO/" + video_name + "/img"
                    one = os.path.join(origDir, f"{newFrameName}.jpg")
                    three = os.path.join(imgDir, f"{newFrameName}.jpg")
                    os.remove(one)
                    os.remove(three)
                    continue
                ...
                frame = np.zeros_like(orig_image_frame)
                temp_frame = np.zeros_like(orig_image_frame)
                ...
                outputDir = self.cfg.video.output_dir + "/_DEMO/" + video_name + "/mask"
                newFrameName = frame_name.split('/')[-1].split('.')[0]
                cv2.imwrite(f'{outputDir}/{newFrameName}.jpg', frame)
                ...
                if (len(detections) == 0):
                    origDir = self.cfg.video.output_dir + "/_DEMO/" + video_name + "/orig"
                    maskDir = self.cfg.video.output_dir + "/_DEMO/" + video_name + "/mask"
                    imgDir = self.cfg.video.output_dir + "/_DEMO/" + video_name + "/img"
                    one = os.path.join(origDir, f"{newFrameName}.jpg")
                    two = os.path.join(maskDir, f"{newFrameName}.jpg")
                    three = os.path.join(imgDir, f"{newFrameName}.jpg")
                    os.remove(one)
                    os.remove(two)
                    os.remove(three)
                ...
                if(self.cfg.render.enable and t_>=self.cfg.phalp.n_init):                    
                    ...
                    for t__ in range(t_, t_+d_):
                        ...
                        if frame_key in final_visuals_dic:
            ...
            self.delete_last_image(video_name)
    ...
    def delete_last_image(self, video_name):
        # List all files in the directory
        outputDir = self.cfg.video.output_dir + "/_DEMO/" + video_name + "/img"
        origDir = self.cfg.video.output_dir + "/_DEMO/" + video_name + "/orig"
        maskDir = self.cfg.video.output_dir + "/_DEMO/" + video_name + "/mask"
        ...
        maskFiles = [f for f in os.listdir(outputDir) if os.path.isfile(os.path.join(maskDir, f))]
        
        # Filter out only image files (you can add more image extensions if needed)
        image_files = [f for f in files if f.endswith('.png') or f.endswith('.jpg') or f.endswith('.jpeg')]
        orig_image_files = [f for f in origFiles if f.endswith('.png') or f.endswith('.jpg') or f.endswith('.jpeg')]
        mask_files = [f for f in maskFiles if f.endswith('.png') or f.endswith('.jpg') or f.endswith('.jpeg')]
        
        # If there are no image files, return
        if len(image_files) == 0 or len(orig_image_files) == 0:
            ...
            last_orig_image = orig_image_files[-1]
            
            # Build the full path of the last image file
            last_image_path = os.path.join(outputDir, last_image)
            last_orig_image_path = os.path.join(origDir, last_orig_image)
        
            # Delete the last image file
            os.remove(last_image_path)
            os.remove(last_orig_image_path)
        elif len(mask_files > len(image_files)):
            mask_files.sort()
            
            # Get the last mask file
            last_image = mask_files[-1]
        ...
        orig_image_files = [f for f in origFiles if f.endswith('.png') or f.endswith('.jpg') or f.endswith('.jpeg')]
        mask_files = [f for f in maskFiles if f.endswith('.png') or f.endswith('.jpg') or f.endswith('.jpeg')]
        assert len(image_files) == len(mask_files) == len(orig_image_files)
