...
class Visualizer(nn.Module):
    ...
    def __init__(self, cfg, hmar):
        ...
        loadingFile = os.path.join(os.path.join(os.environ.get("HOME"), ".cache"), "phalp/3D/smpl_mean_params.npz")
        self.mean_params = np.load(loadingFile)
    ...
    def render_single_frame(self, pred_smpl_params, pred_cam_t, color, img_size = 256, image=None, use_image=False):
        ...
        init_betas = torch.from_numpy(self.mean_params['shape'].astype('float32')).unsqueeze(0)
        pred_smpl_params['betas'] = init_betas
    ...
    def render_video(self, final_visuals_dic):
        ...
        if(len(tracked_smpl)>0):
            ...
            if len(tracked_smpl) >= 2:
                tracked_smpl = np.array([tracked_smpl[-1]])
                tracked_cameras = np.array([tracked_cameras[-1]])
                print("Too many people attempting to render.. Picking the first person")
