...
class IO_Manager():
    ...
    def get_frames_from_source(self, render=False):
        ...
        if(source_path.endswith(".mp4")):
            ...
            os.system("rm -rf " + self.cfg.video.output_dir + "/_DEMO/" + video_name + "/mask/")
            if(self.cfg.video.extract_video):
                os.makedirs(self.cfg.video.output_dir + "/_DEMO/" + video_name, exist_ok=True)
                os.makedirs(self.cfg.video.output_dir + "/_DEMO/" + video_name + "/img", exist_ok=True)
                os.makedirs(self.cfg.video.output_dir + "/_DEMO/" + video_name + "/mask", exist_ok=True)

                fe = FrameExtractor(source_path)
                #log.info('Number of ORIGINAL frames: ' + str(fe.n_frames))
                if render:
                    fe.extract_frames(every_x_frame=1, img_name='', dest_path= self.cfg.video.output_dir + "/_DEMO/" + video_name + "/img/", start_frame=self.cfg.video.start_frame, end_frame=self.cfg.video.end_frame, width=self.cfg.video.width, height=self.cfg.video.height, render=True)
                else:
                    fe.extract_frames(every_x_frame=1, img_name='', dest_path= self.cfg.video.output_dir + "/_DEMO/" + video_name + "/img/", start_frame=self.cfg.video.start_frame, end_frame=self.cfg.video.end_frame, width=self.cfg.video.width, height=self.cfg.video.height)
                list_of_frames = sorted(glob.glob(self.cfg.video.output_dir + "/_DEMO/" + video_name + "/img/*.jpg"))
                list_of_orig_frames = sorted(glob.glob(self.cfg.video.output_dir + "/_DEMO/" + video_name + "/orig/*.jpg"))
            else:
        ...
        io_data = {
            ...
            "list_of_orig_frames": list_of_orig_frames
        }
